...
function renderTable(tableData) {
    if (!tableData || tableData.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="8" class="text-center" style="padding: 40px;">No videos in queue for re-transcoding</td></tr>';
    ...
    const rows = filteredData.map((row, index) => {
        return `
            <tr class="table-row">
                ...
                <td class="text-center">${row.videoSize}</td>
...
function showError(message) {
    tableBody.innerHTML = `<tr><td colspan="8" class="text-center" style="padding: 40px; color: #ef4444;">${message}</td></tr>`;
    tableWrapper.style.display = "block";
}
